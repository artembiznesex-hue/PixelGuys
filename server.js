...
async function loadEarth() {
  ...
  if (!fs.existsSync(earthPath)) {
    console.log('[Canvas] earth.png not found, using blank canvas');
    initChunks();
    loadSavedState();
    return;
  }
  try {
    ...
    // Use native resolution (don't resize - use as-is)
    // Canvas size must be multiple of TILE_SIZE
